...
Java_com_android_facemorphing_DLibResult_setupDlib(
        JNIEnv *env,
        jobject thiz,
...
extern "C"
JNIEXPORT void JNICALL
Java_com_android_facemorphing_DLibResult_processLandMarks(
        JNIEnv *env,
        jobject thiz,
        jobject bitmap,
        jintArray bb
    ...
    // Utilizza il detector per trovare il rettangolo del viso
    std::vector<dlib::rectangle> dets = detector(img);

    if (!dets.empty()) {
        // Utilizza il primo rettangolo del viso trovato
        dlib::rectangle rect = dets[0];

        // Aggiungi i facial landmarks al tuo oggetto Java
